...
function updateDayNightCycle(dt) {
    ...
    if (isNightPhase) {
        targetColor.setHex(0x0a0f2b); targetIntensity = 0.15; baseTemp = -73.0;
    } else {
        targetColor.setHex(0x96321e); targetIntensity = 0.8; baseTemp = -40.0;
    }
    ...
    phobosMat.opacity = starMat.opacity; deimosMat.opacity = starMat.opacity;
...
for (let i = 0; i < 110; i++) {
    ...
    const scale = Math.random() * 2 + 1.5, ry = getGroundHeight(rx, rz);
...
for (let a = 0; a < Math.PI * 2; a += 0.04) {
    const rock = new THREE.Mesh(rockGeo, rockMat);
    const rx = Math.sin(a) * borderRadius, rz = Math.cos(a) * borderRadius;
    const scale = Math.random() * 5 + 10.0, ry = getGroundHeight(rx, rz);
...
body.position.y = 1.0; body.castShadow = true; rover.add(body);
...
for (let pos of wheelOffsets) {
    ...
    w.position.set(pos[0], pos[1], pos[2]); w.castShadow = true;
...
rays.forEach(() => {
    ...
    line.position.y = 0.5; lidarLines.add(line);
});

// --- STATE & INPUTS ---
let throttle = 0.0, steering = 0.0, manualThrottle = 0.0, manualSteering = 0.0, aiThrottle = 0.0, aiSteering = 0.0;
let override = false, autoSpeed = 4.0, autoTurnSpeed = 5.0;
let currentpH = 7.7, currentTemp = -40.0, currentMoisture = 1.0;

function updateEnvironmentStats(dt, payload) {
    if (payload.length === 0) return;
    let crowded = 0; payload.forEach(r => { if (r.distance < 12.0) crowded += (12.0 - r.distance); });
    let norm = Math.min(crowded / 150.0, 1.0); 
    currentpH += (7.7 + norm*0.8 - currentpH) * 1.2 * dt;
    currentTemp += (baseTemp + norm*15 - currentTemp) * 1.2 * dt;
    currentMoisture += (1.0 + norm*2 - currentMoisture) * 1.2 * dt;
    const el = document.getElementById('envData');
    if(el) el.innerHTML = `pH Seviyesi : ${currentpH.toFixed(2)}\nSıcaklık    : ${currentTemp.toFixed(1)}°C\nNem Oranı   : ${currentMoisture.toFixed(1)}%`;
}
...
function updateCamera() {
    const offset = new THREE.Vector3(0, 10, -15).applyQuaternion(rover.quaternion).add(rover.position);
    camera.position.lerp(offset, 0.1);
    const target = new THREE.Vector3(0, 2, 5).applyQuaternion(rover.quaternion).add(rover.position);
    camera.lookAt(target); skyGroup.position.copy(camera.position);
}

// --- TANK-TURN PHYSICS (Turkish Stability) ---
function updatePhysics(dt) {
    ...
    // PHYSICS INTERCEPTOR
    let isBlocked = false, isHazard = false;
    ...
    for (let off of offsets) {
        ...
        if (hits.length > 0) {
            ...
            if (throttle > 0 && d < 7.0) isHazard = true; // Front hazard detection
            if (throttle < 0 && d < 2.5) { isHazard = true; brake = true; } // Rear brake
            if (d < 1.0) { isBlocked = true; break; }
        }
    }

    if (isHazard) {
        assist = true;
        // CLASSIC TANK TURN: stop the rover completely and rotate in place
        throttle = 0; 
    ...
    // TURKISH HUD RESTORATION
    const connectionState = (ws && ws.readyState === WebSocket.OPEN) ? "Bağlandı" : "Bağlantı kesildi (Sunucuyu Başlatın)";
...
function processSensorsAndNetwork(dt) {
    ...
    rays.forEach((ray, i) => {
        ...
        if (!squash[ray.yaw] || dist < squash[ray.yaw]) squash[ray.yaw] = dist;
    });
